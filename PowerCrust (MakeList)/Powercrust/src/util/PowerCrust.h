//Library that uses a PowerCrust algorithm for surface recon
...
#include "util/HCPO3D.h"
#include "util/TrianglesFunctions.h"
#include "util/ArraysLib.h"
#include "util/filemanager.h"
#include "util/predicates.cxx"
#include "util/tetgen.cxx"
#include <queue>
#include <stack>
#include <set>
#include "util/cs.h"
...
class POWERCRUST
{
    ...
    public:
        ...
        int POWERCRUST::TriangulateSpectral(vector<double>* inputp, int innp);
        void FreeMemory();
    ...
    private:
        ...
        /*ABPA*/
        void ABPA();
        ABPAQueueNode POWERCRUST::ABPA_get_seed();
        bool POWERCRUST::TetraIsInfinite(int *tetra);
        int POWERCRUST::TetEdgeRotationForward(int Tid, int p0, int p1);
        int POWERCRUST::TetEdgeRotationBackward(int Tid, int p0, int p1);
        int POWERCRUST::TetEdgeMapOnTetra(int* tet, int p0, int p1);
        void POWERCRUST::ABPA_propagate(ABPAQueueNode *node, ABPA_Queue *queue);
        ABPAQueueNode POWERCRUST::RunPivotOnEdge(int Tin, int Tout, int p1, int p2, bool mark);
        int POWERCRUST::CollectPivotingTetras(int Tin, int Tout, int p1, int p2, vector<int> *forward);
        ABPAQueueNode POWERCRUST::FindMinIfactPivot(vector<int> &tetras, int Tin, bool mark);
        void POWERCRUST::SetTetStatusAndUpdateQueue(int Tid, PC_TET_STATUS status, ABPA_Queue *queue);
        inline double POWERCRUST::IfactFromTetraFaceCC(int T1, int T2);
        void POWERCRUST::FaceCircumCenter(Coord3D* p1, Coord3D* p2, Coord3D* p4, Coord3D* n, double* r, Coord3D* cc);
...
void POWERCRUST::CopyPoints(vector<double>* inputp, int innp)
{
    int i;
    N = innp;
    Nshield = N * .01;
...
void POWERCRUST::AddShield(Coord3D* ps, int n)
{
...
int POWERCRUST::DelaunayTriangulation()
{
...
bool POWERCRUST::ComputeTetraCC(int idT, Coord3D* cc, double* r)
{
...
inline double POWERCRUST::IfactFromCC(Coord3D* cc1, double* r1, Coord3D* cc2, double* r2)
{
...
inline double POWERCRUST::IfactFromTetra(int T1, int T2)
{
    Coord3D cc1, cc2;
    double r1, r2;
    double ifact;
...
void POWERCRUST::AnalyzeIntersection(int T1, int T2)
{
    // T1,T2 is always >0; The marking flow goes from T1 to T2
...
float POWERCRUST::ComputePlevel(float Ifact)
{
    float plevel;
...
void POWERCRUST::FindFacet(int T1, int T2, Triangle* facet)
{
...
void POWERCRUST::MarkTetra(int T1, int T2, float Ifact)
{
...
void POWERCRUST::GenerateTriangles()
{
...
void POWERCRUST::LookForTriangle(int T1, int T2)
{
    Triangle facet;
    bool deleted2;
...
void POWERCRUST::BuildSurface()
{
    ...
    cout << "Gerados : " << t.size() << " triangulos de " << NT << " Tetraedros" << endl;
...
void POWERCRUST::CopyTriangles(int* triangles)
{
    int i = 0;
    int c = 0;
    for (i = 0; i < t.size(); i++)
...
double POWERCRUST::TetraVol(int idT)
{
...
*/
int POWERCRUST::TriangulatePowerCrust(vector<double>* inputp, int innp)
{
    int exitcode;
...
int POWERCRUST::TetEdgeRotationForward(int Tid, int Tprev, int tet_edge_id)
{
...
void POWERCRUST::ABPA_propagate(ABPAQueueNode *node, ABPA_Queue *abpa_queue)
{
    Triangle facet;
...
*/
int POWERCRUST::TriangulateABPA(vector<double>* inputp, int innp)
{
    int exitcode;
...
void POWERCRUST::SpectralPropagation()
{
    double *w = NULL;
    ABPAQueueNode node;
